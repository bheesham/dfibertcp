			...
			if ( !$this->upload->do_upload('upload_torrent') || !$this->form_validation->run() ) {
				...
				if ( $exists_res->num_rows > 0 ) {
					...
					if ( $add_result->result == 'success' ) {
						...
						if ( intval( $row->total_disk_usage + $torrent_size ) < intval( $this->session->userdata('max_total_disk_usage') ) ) {
							$info = array(
							...
							$update = 'UPDATE dft_users SET `total_disk_usage`=total_disk_usage+' . $torrent_size . ' WHERE `uid`=' . intval( $this->session->userdata('uid') ) . ';';
							...
							$this->TransmissionRPC->remove( $add_result->arguments->torrent_added->id, TRUE );
							unlink( $torrent['full_path'] );
							$data['error'] = 'By adding that torrent, you will exceed your disk usage.';
						}
					} else {
						unlink( $torrent['full_path'] );
